...
ref_system_mda = mda.Universe("groel_holo.pdb") # The system used to create the experimental images (always needed!)
ref_system_mda = center_system(ref_system_mda) # Center your coordinates if they are not centered

# *********************************************** From random noise ********************************************************
sigma = 0.5
select_filter = "name CA" # Change if needed (defines which atoms are selected from the pdb)
...
# sim_system_mda = mda.Universe("groel_apo.pdb")
...
N_STEPS = 10000; STRIDE = 1000; LEARN_RATE = 0.001; TOL = -1#1e-8 # Parameters for gradient descent (TOL=-1 means it does all the steps)
...
MPI_RANKS = 1
OUT_PFX = "groel_final_" # final coordinates saved as OUT_PFX + coords.txt
NTOMP = 1
